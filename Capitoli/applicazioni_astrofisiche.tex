...
\begin{equation}
  ...
  \frac{r^2}{2}(\theta_2 - \sin\theta_2).
\end{equation}
Se $r_\star - r \leq d < \sqrt{r_\star^2 - r^2}$ l'area di sovrapposizione diventa
\begin{equation}
  \delta A = \pi r^2 - S_1  + S_2 = \frac{r_\star^2}{2}(\theta_1 - \sin\theta_1) +
...
\begin{equation}
  ...
  \begin{dcases}
    ...
    \sin\theta_2) & \text{se $\sqrt{r_\star^2 - r^2} \leq d \leq r_\star + r$},
    \\
    \frac{r_\star^2}{2}(\theta_1 - \sin\theta_1) + \frac{r^2}{2}(2\pi - \theta_2 +
    \sin\theta_2) & \text{se $r_\star - r \leq d < \sqrt{r_\star^2 - r^2}$}, \\
...
si misura in unità di \si{energia\per tempo}, quindi nel sistema internazionale
\si{\joule \per \second}, mentre nel CGS si userà \si{erg\per
  \second}. Definiamo il \emph{flusso superficiale} $F_\star$ come
\begin{equation}
