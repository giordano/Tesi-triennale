...
dell'eclissi è
\begin{equation}
  \Delta t \approx \frac{P}{2\pi a}2(r_1 + r_2) = \frac{P(r_1 + r_2)}{\pi a}.
\end{equation}
...
\begin{figure}
  ...
  {\begin{tikzpicture}[scale=0.7,font=\footnotesize]
      ...
      \draw (-\xingresso,\acosi) circle (\rdue) % posizione di ingresso iniziale
            ...
            (0,0) -- node[sloped,below,fill=white] {$r_1 + r_2$}
            (-\xingresso,\acosi);
...
$t_{\textup{ef}}$ è circa $2\sqrt{(r_1 + r_2)^2 - a^2\cos^2 i}$, quindi la
durata dell'eclissi in questo caso è
\begin{equation}
  \Delta t \approx \frac{P}{2\pi a} 2\sqrt{(r_1 + r_2)^2 - a^2\cos^2 i} =
