...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \draw (O1) circle (\runo); % disco stella
    \node at ($(O1) + (2.3,-1.2)$) {$m_1$};
    \draw (O2) circle (\rdue); % disco pianeta
    \node at ($(O2) + (-1.1,-1)$) {$m_2$};
    \draw[->] (O1) -- (0,3) node[above] {$z$}; % asse z
    \draw[->] (O1) -- (10,0) node[right] {$x$}; % asse x
    % asse x''
    \draw[->] (O1) -- ($cos((\i)*(10,0) + sin(\i)*(0,10)$) node[right] {$x''$};
    \draw[->,dashed] ($(O1) + (0,2.5)$) to[out=0,in=90+\i] node[right=5]
                     {$i_{\textup{min}}$} ($cos(\i)*(2.5,0) +
                     sin(\i)*(0,2.5)$); % angolo di inclinazione
    ...
    \draw (O1) -- node[below right] {$r_\star$} ($(O1) - cos(45)*(2,0) -
          sin(45)*(0,2)$);
    \draw (O2) -- node[right] {$r$} ($(O2) + cos(90+\i)*(1,0) +
          sin(90+\i)*(0,1)$);
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=0.6]
    ...
    \draw (-\runo-\rdue,0) -- node[above] {$r$}
          ($(-\runo-\rdue,0)-cos(45)*(\rdue,0)-sin(45)*(0,\rdue)$);
...
\begin{figure}
  ...
  {\begin{tikzpicture}[scale=0.7]
      ...
      \draw (-\xingresso,\acosi) circle (\rdue) % posizione di ingresso iniziale
            ...
            (-\xingresso,\acosi);
      \draw[dashed] ($-\xingresso*(1,0) + (0,\acosi)$) --
                    ($-\xingresso*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ii}}$}
                    ($\xingresso*(1,0) + (0,\acosi)$) --
                    ($\xingresso*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ef}}$};
      \draw (-\xegresso,\acosi) circle (\rdue) % posizione di ingresso finale
            ...
            (0,0) -- node[sloped,below,fill=white] {$r_\star-r$}
            (\xegresso,\acosi);
      \draw[dashed] ($-\xegresso*(1,0) + (0,\acosi)$) --
                    ($-\xegresso*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{if}}$}
                    ($\xegresso*(1,0) + (0,\acosi)$) --
                    ($\xegresso*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ei}}$};
