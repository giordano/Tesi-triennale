...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \draw (O1) circle (2); % disco stella (r=2)
    \node at ($(O1) + (2.3,-1.2)$) {$m_1$};
    \draw (O2) circle (1); % disco pianeta (r=1)
    \node at ($(O2) + (-1.1,-1)$) {$m_2$};
    \draw[->] (O1) -- (0,3) node[above] {$z$}; % asse z
    \draw[->] (O1) -- (10,0) node[right] {$x$}; % asse x
    \draw[->] let \n1 = {acos(sqrt(1-1*1/(8*8)))} in
                  (O1) -- ($cos((\n1)*(10,0) + sin(\n1)*(0,10)$) node[right]
                  {$x''$}; % asse x''
    \draw[->,dashed] let \n1 = {acos(sqrt(1-1*1/(8*8)))} in
                         ($(O1) + (0,2.5)$) to[out=0,in=90+\n1] node[right=5]
                         {$i_{\textup{min}}$} ($cos(\n1)*(2.5,0) +
                         sin(\n1)*(0,2.5)$); % angolo di inclinazione
    % distanza fra i centri di pianeta e stella
    \draw[<->] ($(O1) + (0,-0.2)$) -- node[fill=white] {$a$} ($(O2) + (0,-0.2)$);
    \draw (O1) -- node[below right] {$r_\star$} ($(O1) - cos(45)*(2,0) -
                                    sin(45)*(0,2)$);
    \draw let \n1 = {acos(sqrt(1-1*1/(8*8)))} in
              (O2) -- node[right] {$r$} ($(O2) + cos(90+\n1)*(1,0) +
              sin(90+\n1)*(0,1)$);
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=0.6]
    ...
    \draw (O) circle (4); % disco stella
    \draw (O) -- node[right] {$r_\star$} (0,-4);
    \draw (-5,0) -- node[above] {$r$} ($(-5,0)-cos(45)*(1,0)-sin(45)*(0,1)$);
    \foreach \x in {-5,-3,-1,1,3,5} % varie posizioni del pianeta
      \draw (\x,0) circle (1);
    \draw (-8,-3) node[left] {luminosità} -- (-5,-3) -- (-3,-5) -- (3,-5) --
          (5,-3) -- (8,-3); % curva di luce
    \draw[dashed] (-5,0)  -- (-5,-6) node[below] {$t_{\textup{ii}}$};
    \draw[dashed] (-4,-4) -- (-4,-6) node[below] {$t_{\textup{mi}}$};
    \draw[dashed] (-3,0)  -- (-3,-6) node[below] {$t_{\textup{if}}$};
    \draw[dashed] (3,0)   -- (3,-6)  node[below] {$t_{\textup{ei}}$};
    \draw[dashed] (4,-4)  -- (4,-6)  node[below] {$t_{\textup{mf}}$};
    \draw[dashed] (5,0)   -- (5,-6)  node[below] {$t_{\textup{ef}}$};
    \draw[->] (-8,-6) -- (8,-6) node[right] {$t$}; % asse del tempo
  \end{tikzpicture}
...
\begin{figure}
  ...
  {\begin{tikzpicture}[scale=0.7]
      \draw (4,0) arc (0:180:4); % disco stella
      \draw (-6,2.5) -- (6,2.5); % asse passante lungo equatori dei pianeti
      \draw (-4,0) -- (4,0); % diametro stella
      \draw[<->] (0,0) -- node[sloped,above,fill=white] {$a\cos i$} (0,2.5);
      \draw[->] (-6,-1) -- (6,-1) node[right] {$t$}; % asse del tempo
      \draw let \n1 = {sqrt((4+1)^2-2.5^2)} in
                (-\n1,2.5) circle (1) % posizione di ingresso iniziale
                (\n1,2.5)  circle (1) % posizione di egresso finale
                (0,0) -- node[sloped,below,fill=white] {$r_\star + r$} (-\n1,2.5);
      \draw[dashed] let \n1 = {sqrt((4+1)^2-2.5^2)} in
                    ($-\n1*(1,0) + (0,2.5)$) --
                    ($-\n1*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ii}}$}
                    ($\n1*(1,0) + (0,2.5)$) --
                    ($\n1*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ef}}$};
      \draw let \n2 = {sqrt((4-1)^2-2.5^2)} in
                (-\n2,2.5) circle (1) % posizione di ingresso finale
                (\n2,2.5)  circle (1) % posizione di egresso iniziale
                (0,0) -- node[sloped,below,fill=white] {$r_\star-r$} (\n2,2.5);
      \draw[dashed] let \n2 = {sqrt((4-1)^2-2.5^2)} in
                    ($-\n2*(1,0) + (0,2.5)$) --
                    ($-\n2*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{if}}$}
                    ($\n2*(1,0) + (0,2.5)$) --
                    ($\n2*(1,0) + (0,-1)$) node[below]
                    {$t_{\textup{ei}}$};
  ...
  {\begin{tikzpicture}[scale=0.7]
      ...
      \draw[->,dashed] let \n1 = {atan(0.1)} in
                           ...
                           {$i$} ($cos(\n1)*(2.5,0) +
                           sin(\n1)*(0,2.5)$);
      \draw (O1) -- node[left] {$r_\star$} ($(O1) - cos(45)*(2,0) -
                                      sin(45)*(0,2)$);
      \draw (O2) -- node[right] {$r$} ($(O2) + cos(45)*(1,0) - sin(45)*(0,1)$);
    \end{tikzpicture}}
  \caption{Transito di un pianeta davanti alla stella compagna nel caso
    $i \neq \pi/2$}
...
\begin{figure}
  ...
  {\begin{tikzpicture}[scale=1.1]
      ...
      \draw[name path=S] (O1) circle (2); % disco stella (r=2)
      \draw[name path=P] (O2) circle (1); % disco pianeta (r=1)
      % individuo punti di intersezione fra dischi di pianeta e stella
      \path[name intersections={of=S and P,by={A,B}}];
      \draw[dashed] (O1) -- node[above] {$r_\star$} (A) -- (O2) --
                    node[below] {$r$} (B) -- (O1);
      \draw (A) -- (B);
  ...
  {\begin{tikzpicture}[scale=1.1]
      ...
      % individuo punti di intersezione fra dischi di pianeta e stella
      \path[name intersections={of=S and P,by={A,B}}];
      \draw[dashed] (O1) -- node[below] {$r_\star$} (A) -- (O2) --
                    node[left] {$r$} (B) -- (O1);
      \draw (A) -- (B);
      \draw[->] ($(B) - (0,0.5)$) node[left,fill=white] {$S_1$} to
                [out=0,in=-60] (1.9,-0.1);
