...
Calcoliamo l'area della stella coperta dal pianeta durante l'eclissi. La
distanza che prenderemo in considerazione non sarà la distanza reale $a$ fra i
due corpi ma quella proiettata $d$ nel piano del cielo dell'osservatore e
introdotta nel paragrafo~\ref{sec:geometria-sistema}. Se in un certo istante di
tempo si ha $d > R + r$, i due corpi non sono sovrapposti nel cielo
dell'osservatore quindi non si sta verificando l'eclissi. Se invece
$d \leq R + r$ il pianeta copre parzialmente la stella. Finché
$\sqrt{R^2 - r^2} \leq d \leq R + r$, l'area di sovrapposizione è la somma delle
due aree $S_1$ e $S_2$ della
...
è dato da
\begin{equation}
  \theta_1 = 2 \arccos \frac{R^2 - r^2 + d^2}{2Rd}
\end{equation}
e analogamente per l'angolo $\theta_2$ abbiamo
\begin{equation}
  \theta_2 = 2 \arccos \frac{r^2 - R^2 + d^2}{2rd}.
\end{equation}
...
\begin{subequations}
  \begin{align}
    ...
    S_2 &= \frac{\theta_1}{2\pi}\pi R^2 - \frac{R^2}{2}\sin\theta_1 =
    \frac{R^2}{2}(\theta_1 - \sin\theta_1).
  \end{align}
\end{subequations}
Dunque in questo caso l'area di sovrapposizione $A(d)$ vale
\begin{equation}
  A(d) = S_1 + S_2 = \frac{R^2}{2}(\theta_1 - \sin\theta_1) +
  \frac{r^2}{2}(\theta_2 - \sin\theta_2).
\end{equation}
Se $R - r \leq d < \sqrt{R^2 - r^2}$ l'area di sovrapposizione diventa
\begin{equation}
  A(d) = \pi r^2 - S_1  + S_2 = \frac{R^2}{2}(\theta_1 - \sin\theta_1) +
  \frac{r^2}{2}(2\pi - \theta_2 + \sin\theta_2).
\end{equation}
Infine se $d < R - r$ significa che il pianeta si trova completamente davanti
alla stella rispetto all'osservatore quindi l'area di sovrapposizione è uguale
all'area del pianeta vista dall'osservatore stesso, cioè
\begin{equation}
  A(d) = \pi r^2.
\end{equation}
Riepilogando
\begin{equation}
  ...
  \begin{dcases}
    0 & \text{se $d > R + r$}, \\
    \frac{R^2}{2}(\theta_1 - \sin\theta_1) + \frac{r^2}{2}(\theta_2 -
    \sin\theta_2) & \text{se $\sqrt{R^2 - r^2} \leq d \leq R + r$}, \\
    \frac{R^2}{2}(\theta_1 - \sin\theta_1) + \frac{r^2}{2}(2\pi - \theta_2 +
    \sin\theta_2) & \text{se $R - r \leq d < \sqrt{R^2 - r^2}$}, \\
    \pi r^2 & \text{se $d < R - r$}.
  \end{dcases}
\end{equation}
...
esattamente quella della stella.
