...
detta \emph{equazione di Keplero}.\footnote{In letteratura è usata spesso una
  notazione differente: con $E$ si indica l'anomalia eccentrica, con $M$
  l'anomalia media e con $n$ la velocità angolare media. In questo modo
  l'equazione di Keplero assume l'espressione $M \equiv n(t-t_{0}) = E - e\sin
  E$.} In ogni fissato istante di tempo $t$, la sua soluzione $\psi$ permette di
ricavare, tramite la~\eqref{eq:r-anomalia-eccentrica}, la coordinata polare
$r$. Inoltre possiamo determinare una relazione tra l'anomalia eccentrica e
l'anomalia vera $\theta$. Infatti, uguagliando i secondi membri delle
equazioni~\eqref{eq:orbita} e~\eqref{eq:r-anomalia-eccentrica} abbiamo
\begin{equation}
  \begin{aligned}
...
\begin{subequations}
  \begin{gather}
    ...
    J_n'(x) = \frac{J_{n-1}(x) - J_{n+1}(x)}{2} = \frac{n}{x}J_n(x) - J_{n+1}(x) =
    J_{n-1}(x) - \frac{n}{x}J_n(x),\\
...
\begin{equation}
  \toder{\psi}{\phi} = 1 + \sum_{n=1}^{+\infty}2J_n(ne)\cos(n\phi).
\end{equation}
