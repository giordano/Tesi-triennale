...
di Newton~-~Raphson (si veda \textcite[30]{brugnano:calcolo-numerico}), chiamato
anche \emph{metodo delle tangenti}, descritto nel seguente
...
espansa in serie di Laurent di potenze di $z$ intorno a $z=0$
\begin{equation}
...
\begin{equation}
  ...
  \quad n\in\Z,
\end{equation}
in cui $\gamma$ è un qualsiasi circolo centrato nell'origine. Si dimostra
che (si veda \textcite[355]{whittaker:modern-analysis})
\begin{equation}
...
La funzione $J_n(x)$ è chiamata \emph{funzione di Bessel di argomento $x$ e
  ...
  funzione di Bessel}. Si dimostrano inoltre le seguenti formule ricorsive (si
vedano \textcites[17]{watson:bessel}[359]{whittaker:modern-analysis})
\begin{subequations}
  \begin{gather}
    \frac{2n}{x} J_n(x) = J_{n-1}(x) + J_{n+1}(x),\\
    \toder{J_n(x)}{x} = \frac{J_{n-1}(x) - J_{n+1}(x)}{2} = \frac{n}{x}J_n(x) -
    J_{n+1} = J_{n-1} - \frac{n}{z}J_n,\\
    \toder[2]{J_n(x)}{x} = \frac{n^2}{x^2}J_n(x) - J_n(x) -
    \frac{1}{x}\toder{J_n(x)}{x},\\
    \toder{}{x}(x^nJ_n(x)) = x^nJ_{n-1}(x),\\
    \toder{}{x}(x^{-n}J_n(x)) = -xJ_{n+1}(x).
  \end{gather}
\end{subequations}
Si può verificare grazie relazioni appena riportate che la funzione $J_n(x)$
soddisfa la seguente equazione differenziale nell'incognita $y(x)$
\begin{equation}
...
2\pi\mathclose{]}^2)$, quindi per il teorema del Dini (si veda
...
cercare un'espressione che contiene i coefficienti di Bessel, che sono funzioni
...
sviluppabile in serie di Fourier (si veda \textcite[440]{demarco:analisi2}) in
...
\end{equation}
Si dimostra che (si veda \textcite[555]{watson:bessel})
\begin{equation}
...
calcolato con il metodo delle funzioni di Bessel le stesse grandezze calcolate
...
\begin{figure}
  ...
  \caption[Anomalia eccentrica in funzione del tempo con il metodo delle
  funzioni di Bessel]{Anomalia eccentrica in funzione del tempo calcolata
    utilizzando il metodo delle funzioni di Bessel}
  \label{fig:bessel-anomalia_eccentrica}
\end{figure}
\begin{figure}
  \centering
  \input{keplero/bessel-raggio}
  \caption[Distanza dal fuoco in funzione del tempo con il metodo delle funzioni
  di Bessel]{Distanza dal fuoco in funzione del tempo calcolata con
...
\begin{figure}
  ...
  \caption[Anomalia vera in funzione del tempo con il metodo delle funzioni di
  Bessel]{Anomalia vera in funzione del tempo calcolata con
    la~\eqref{eq:anoamlie-vera-eccentrica} dopo aver trovato l'anomalia
    eccentrica con il metodo delle funzioni di Bessel}
