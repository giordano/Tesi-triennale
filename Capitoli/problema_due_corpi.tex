...
\begin{figure}
  \centering
  % Ruoto sistema di riferimento principale
  \tdplotsetmaincoords{70}{135}
  \begin{tikzpicture}[tdplot_main_coords,scale=3,font=\scriptsize]
...
$l_0 = \norm{\bm{l}_0} = \norm{\bm{r} \times \mu\dot{\bm{r}}} = \mu
r^2\dot{\theta}$ del momento angolare. Infatti
\begin{equation}
...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\footnotesize,scale=2]
    \input{Immagini/tikz/area_spazzata}
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\scriptsize,scale=5]
    \input{Immagini/tikz/ellisse_polare}
  \end{tikzpicture}
...
Ricordiamo alcune proprietà geometriche delle coniche. Per l'ellisse (si veda la
...
{\allowdisplaybreaks{\begin{equation}
  ...
  \begin{split}
    ...
    \right).
  \end{split}
\end{equation}}Questa relazione lega il modulo quadro della velocità alla
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[scale=2.5,font=\footnotesize]
    \input{Immagini/tikz/ellisse_los}
  \end{tikzpicture}
...
coerentemente col fatto che per il moto circolare la velocità è sempre
...
\begin{figure}
  \centering
  \begin{tikzpicture}[xscale=1.45,yscale=0.1,font=\scriptsize]
    \input{Immagini/tikz/potenziale_efficace}
  \end{tikzpicture}
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
Poiché non sono presenti vincoli olonomi e ciascuno dei vettore $\bm{r}_{1}$ e
$\bm{r}_{2}$ è descritto da tre coordinate indipendenti il numero di gradi di
libertà del sistema è pari a sei. Supponiamo che le forze con cui interagiscono
i due corpi derivino da un unico potenziale $U=U(\bm{r})$ dipendente solo dalla
posizione relativa fra i due corpi, dunque la lagrangiana del sistema è data da
\begin{equation}
...
riferimento. Abbiamo nuovamente trovato che il problema dei due corpi diventa lo
studio del moto di una particella libera, quella del centro di massa che abbiamo
fissato nell'origine del sistema di riferimento, e della particella relativa in
un campo esterno. Il vettore $\bm{r}$ svolge dunque il ruolo di coordinate
generalizzate e poiché nello spazio è individuato da tre coordinate il numero di
gradi di libertà del sistema si riduce in questo modo a tre. L'equazione di
Lagrange per la coordinata $\bm{r}$ è
\begin{equation}
...
due corpi e non dalla direzione del vettore $\bm{r}$, cioè $U=U(r)$, e che la
direzione lungo la quale agisce la forza sia quella del vettore $\bm{r}$, cioè
che la forza relativa sia centrale. Come abbiamo visto nel
paragrafo~\ref{sec:problema-keplero}, in questo caso il vettore momento angolare
$\bm{l}_0$ è un integrale primo e il moto si svolge in un piano. Allora sono
sufficienti due coordinate per studiare il moto della particella relativa e
possiamo riscrivere la lagrangiana~\eqref{eq:lagrangiana2-due-corpi} nelle
coordinate polari $r,\theta$
\begin{equation}
...
\end{equation}
è una costante del moto. Abbiamo così trovato di nuovo che il modulo del vettore
momento angolare è costante e ricordiamo che, grazie a questo risultato, nel
...
\begin{equation}
  \begin{split}
    ...
    \frac{1}{2\mu} \left( p^{2} + \frac{p_{\theta}^{2}}{r^{2}} \right) -
...
\begin{figure}
  ...
  \pgfmathsetmacro{\muno}{5} % m_1=5
  \pgfmathsetmacro{\mdue}{1} % m_2=1
  \pgfmathsetmacro{\mridotta}{\muno*\mdue/(\muno+\mdue)} % mu=m_1*m_2/(m_1+m_2)

  \pgfmathsetmacro{\auno}{\a*\mridotta/\muno} % a_1=a*mu/m_1
  \pgfmathsetmacro{\buno}{\b*\mridotta/\muno} % b_1=b*mu/m_1
  \pgfmathsetmacro{\cuno}{\auno*\e} % c_1=a_1*e
  \pgfmathsetmacro{\adue}{\a*\mridotta/\mdue} % a_2=a*mu/m_2
  \pgfmathsetmacro{\bdue}{\b*\mridotta/\mdue} % b_2=b*mu/m_2
  \pgfmathsetmacro{\cdue}{\adue*\e} % c_2=a_2*e
  \subfloat[][Orbite ellittiche dei due corpi nel sistema di riferimento del
  centro di massa]
  {\begin{tikzpicture}[font=\footnotesize,scale=2.5]
      \input{Immagini/tikz/soluzione_due_corpi}
    \end{tikzpicture}} \quad
  \subfloat[][Orbita della particella relativa nel sistema di riferimento con
  origine nel corpo di massa $m_1$]
  {\begin{tikzpicture}[font=\footnotesize,scale=2.5]
      \input{Immagini/tikz/soluzione_particella_relativa}
    \end{tikzpicture}}
...
$n$ gradi di libertà esistono al massimo $2n-1$ integrali primi del moto
indipendenti (si veda \textcite[42]{landau:meccanica}). Se il sistema ha $n$
costanti indipendenti si dice \emph{integrabile}, se queste sono più di $n$ si
dice \emph{superintegrabile}, se sono esattamente $2n-1$ il sistema sarà
\emph{massimalmente superintegrabile}. Abbiamo visto che il problema di Keplero
ha tre gradi di libertà e finora abbiamo trovati quattro costanti: l'energia $E$
...
Adesso conosciamo sette integrali primi del moto, ma non sono tutti fra loro
indipendenti. Un prima relazione che lega queste costanti è l'ortogonalità fra
$\bm{A}$ e $\bm{l}_0$, l'altra deriva da $A = \mu ke$. Infatti, ricordando la
relazione~\eqref{eq:eccentricita-energia} fra l'eccentricità $e$ e l'energia $E$
risulta $A^2 = \mu^2k^2e^2 = \mu^2k^2 + 2\mu El_0^2$. Questo prova l'esistenza
di cinque costanti indipendenti fra le sette complessivamente trovate e il
sistema è massimalmente superintegrabile.
