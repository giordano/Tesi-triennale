...
\begin{subequations}
  \begin{align}
    ...
    \left( -\frac{1}{p}e\sin(\theta-\tilde\omega)
    \right) = \frac{l_0}{\mu p}e \sin(\theta -
    \tilde\omega), \label{eq:velocita-radiale}\\
...
\end{figure}
Nelle osservazioni astronomiche di un sistema composto da due corpi celesti in
moto su orbite ellittiche non sempre è possibile misurare direttamente la
velocità totale il cui modulo quadro è dato dalla~\eqref{eq:velocita-ellisse},
ma molto spesso è possibile misurare la sua componente $v_\textup{los}$ nella
direzione di vista dell'osservatore. Supponiamo che l'osservatore guardi il
sistema dei due corpi nella direzione dell'asse $\versor{x}$, come nella
figura~\ref{fig:ellisse-vlos}. Senza perdita di generalità possiamo porre, per
semplicità, $\tilde\omega=0$. Per calcolare $v_\textup{los}$ facciamo le
seguenti considerazioni geometriche. Chiamiamo $\alpha(\theta)$ l'angolo
compreso fra la direzione di $\versor{r}(\theta)$ e del vettore
$\dot{\bm{r}}(\theta) \equiv \bm{v}(\theta)$ e sia per il momento $\theta \in
\mathopen{[}0, \pi\mathclose{]}$. La tangente di questo angolo è data dal
rapporto fra la componente tangenziale $r(\theta)\dot{\theta}$ della velocità e
la componente radiale $\dot{r}(\theta)$, quindi
\begin{equation}
...
accorgimento. La funzione $\arctan$ ha codominio $\mathopen{]}-\pi/2,
\pi/2\mathclose{[}$, ma nell'intervallo $\theta \in \mathopen{]}\pi,
2\pi\mathclose{[}$ la componente radiale $\dot{r}(\theta)$ assume valori
negativi, come si può vedere dalla~\eqref{eq:velocita-radiale} e nella
figura~\ref{fig:ellisse-vlos}, quindi l'angolo $\alpha(\theta)$ che la velocità
forma con la direzione radiale positiva è maggiore di $\pi/2$. Per ottenere il
valore giusto di $\alpha(\theta)$ in questo intervallo dobbiamo aggiungere $\pi$
al risultato dato dalla~\eqref{eq:angolo-alpha}, quindi in definitiva
\begin{equation}
  ...
  \begin{cases}
    \arctan
     \left(
    \cot\theta + \dfrac{\csc\theta}{e}
  \right) & \text{se } \theta \in \mathopen{[}0, \pi\mathclose{]},\\[2ex]
  \arctan
  \left(
    \cot\theta + \dfrac{\csc\theta}{e}
...
in $\theta = \pi$. Indichiamo con $\beta(\theta)$ l'angolo compreso fra la
direzione del vettore velocità $\bm{v}(\theta)$ e la linea di vista, nel nostro
caso l'asse $\versor{x}$. Poiché $\alpha(\theta)$ è l'angolo fra il verso
positivo della direzione radiale e il vettore $\bm{v}(\theta)$ e la direzione
radiale forma un angolo $\theta$ con la direzione positiva dell'asse
$\versor{x}$, abbiamo $\beta(\theta) = \theta + \alpha(\theta)$ e la componente
...
\begin{figure}
  ...
  dell'anomalia]{Andamento della velocità lungo la direzione della linea di
    vista del corpo di massa ridotta in funzione dell'angolo $\theta$ nel caso
    di orbita ellittica. Il grafico è stato realizzato per gli stessi valori di
    eccentricità utilizzati nella figura~\ref{fig:velocita}, con $a=1$ e
    $\tilde\omega=0$}
...
circolari, si ha sempre $\alpha(\theta) = \pi/2$, coerentemente col fatto che
...
funzione dell'angolo $\theta$ per diversi valori dell'eccentricità.
...
\tilde\omega$, perché il sistema è maggiormente legato in questo punto, pertanto
\begin{equation}
...
cioè $\dot{\bm{r}}_\textup{g} = \bm{0}$, e così la lagrangiana diventa
\begin{equation}
...
\begin{figure}
  ...
  \caption[Orbite dei due corpi]{Nella figura di sinistra sono riportate le orbite
    ellittiche dei due corpi nel sistema di riferimento del centro di
    massa. Nella figura di destra è rappresentata l'orbita della particella
