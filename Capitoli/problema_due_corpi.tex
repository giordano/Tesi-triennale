...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\scriptsize,scale=5]
    ...
    \draw[->] (0.9,0) to [out=90,in=-45] node[right] {$\theta - \theta_0$}
              ($(0.8,0) + cos(45)*(0.1,0) + sin(45)*(0,0.1)$);
...
Si dimostra che in un sistema con $n$ gradi di libertà esistono $2n+1$ integrali
primi del moto indipendenti. Abbiamo visto che il problema di Keplero ha due
gradi di libertà e finora abbiamo trovati quattro costanti: l'energia e le tre
componenti del vettore momento angolare $\bm{l}_0$. Per tutti i campi centrali
con forza del tipo $\bm{F} = -k/r^2\versor{r}$, agente su un corpo di massa
$\mu$, esiste un altro vettore costante chiamato
\emph{vettore di Laplace-Runge-Lenz} e definito da
\begin{equation}
  \bm{A} = \bm{p}\times\bm{l} - \mu k\versor{r}.
\end{equation}
Per il campo gravitazionale naturalmente abbiamo $k=GM_\textup{T}\mu$, dove
$M_\textup{T}$ è la massa del corpo che genera il campo. Verifichiamo che il
vettore $\bm{A}$ è una costante del moto
\begin{equation}
  \begin{split}
...
gli assi $x$ e $y$ della figura~\ref{fig:ellisse}. Consideriamo ora il sistema
di riferimento ausiliario $Oxyz$, ottenuto ruotando attorno all'asse
$z\equiv z'$ il sistema $Ox'y'z'$ di un angolo $\phi_0$. Le coordinate nel
sistema di riferimento ausiliario possono essere ottenute da quelle del sistema
di riferimento intrinseco con la seguente trasformazione scritta in forma
matriciale (si veda~\textcite{goldstein:meccanica})
\begin{equation}
  \begin{pmatrix}
    x \\
    y \\
...
angolo $i$ con la direzione dell'asse $z\equiv z'$. Consideriamo dunque un nuovo
sistema di riferimento destrorso $Ox''y''z''$, ottenuto ruotando attorno
all'asse $y''\equiv y$ il sistema $Oxyz$ di un angolo $\pi/2 - i$. La
trasformazione che permette di passare dal sistema di riferimento ausiliario a
questo nuovo sistema è la seguente
\begin{equation}
  \begin{pmatrix}
    x'' \\
    y'' \\
...
fra i due corpi nel piano del cielo. Se indichiamo con $(y'',z'')$ le coordinate
della particella relativa nel piano del cielo allora la distanza proiettata vale
\begin{equation}
  d = \sqrt{(y'')^2 + (z'')^2}.
\end{equation}
...
due corpi conoscendo quelle della particella relativa. Indicando con
