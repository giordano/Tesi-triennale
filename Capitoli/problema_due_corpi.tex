...
\begin{figure}
  ...
  \begin{tikzpicture}[tdplot_main_coords,font=\footnotesize,scale=3]
    ...
    \draw [->,thick] (O) -- node[left,fill=white] {$\bm{r}_1$}
...
\begin{figure}
  ...
  \begin{tikzpicture}[font=\scriptsize,scale=5]
    ...
    \draw (\c,0) node[below left] {$O\equiv M_{\textup{T}}$};
    \draw[->] (\c+0.1,0) to [out=90,in=-45] node[inner sep=0,fill=white,right=3]
              {$\theta - \theta_0$} ($(\c,0) + cos(45)*(0.1,0) +
              sin(45)*(0,0.1)$);
    \draw[->] (\c,0) -- node[above] {$r$} (\c+0.162586,0.162586);
    \draw[<->] (0,0.05) -- node[fill=white] {$a$} (-\a,0.05);
    \draw[<->] (0,0.05) -- node[fill=white] {$c$} (\c,0.05);
    \draw[<->] (-0.05,0) -- node[fill=white] {$b$} (-0.05,\b);
    \draw[<->] (\c-0.05,0) -- node[fill=white] {$p$} (\c-0.05,\p);
    \draw[<->] (\c,-0.05) -- node[fill=white,inner sep=0] {$a-c$} (\a,-0.05);
  \end{tikzpicture}
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[scale=2.5,font=\footnotesize,
    ...
    % La funzione restituisce valori corretti se #1 != 0, 180, 360 e numeri a
    % loro congrui in modulo 360. Poniamo GM_T = 1 (quindi l_0/mu=sqrt(p)).
    ...
    \draw ({\xuno+0.2*cos(\anomaliauno)},{\yuno+0.2*sin(\anomaliauno)}) arc
          ({\anomaliauno}:beta(\anomaliauno):0.2) node[above=3] {$\alpha$};
    \draw[loosely dashed] (O) -- (\xuno,\yuno);
    \draw ($(O) + (0.2,0)$) arc (0:{\anomaliauno}:0.2) node[right=5] {$\theta$};
    %% secondo punto
    % vettore velocità
...
\bm{v}_{\textup{r}}\versor{r} + \bm{v}_{\textup{t}}\versor{\theta}$ e sia per il
momento $\theta \in \mathopen{[}0, \pi\mathclose{]}$. La tangente di questo
angolo è data dal rapporto fra la componente tangenziale $r(\theta)\dot{\theta}$
della velocità e la componente radiale $\dot{r}(\theta)$, quindi
\begin{equation}
...
funzione dell'anomalia $\theta$ per diversi valori dell'eccentricità.
...
di cinque costanti indipendenti fra le sette complessivamente trovate.
