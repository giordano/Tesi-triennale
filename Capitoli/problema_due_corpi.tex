...
\begin{subequations}
  \begin{align}
    ...
    e &= \frac{c_0p}{\cos\tilde\omega}.
  \end{align}
\end{subequations}
Dunque fissate $c_0$ e $c_1$, $\tilde\omega$ ed $e$ sono univocamente
...
\begin{figure}
  ...
  \caption{Ellisse in coordinate polari. L'origine del sistema di riferimento è
    ...
    è l'eccentricità lineare. Nel grafico abbiamo posto $a=1$ ed $e=0.7$.}
  \label{fig:ellisse}
\end{figure}
...
vista %TODO: aggiungere immagine come la 47 dello Smart e completare il
      %discorso!
...
\begin{figure} %TODO: scegliere valori migliori per il grafico
  ...
  \caption{Andamento qualitativo del potenziale efficace nel problema di Keplero
    ...
    funzione $k_1/r^2-k_2/r$, con $k_1$ e $k_2$ costanti positive scelte
    arbitrariamente.}
...
\subsubsection{Esercizio} %TODO: titolo
\label{sec:esercizio}
...
% TODO: c'è bisogno di differenziare? Non si vede subito dall'espressione di E?
\begin{equation}
...
Un sistema fisico composta da $N$ particelle nello spazio è descritto da $3N$
coordinate, ma se il sistema è soggetto a dei vincoli queste $3N$ grandezze non
sono tutte necessariamente indipendenti fra loro. Se i vincoli sono
olonomi\footnote{Un vincolo si dice \emph{olonomo} se è esprimibile sotto forma
  di un'equazione del tipo $f(\bm{r}_1,\dots,\bm{r}_N,t)=0$, dove le $\bm{r}_i$
  rappresentano le posizioni delle $N$ particelle all'istante $t$.} ed espressi
da $k$ equazioni allora è possibile introdurre $n=3N-k$ coordinate indipendenti
che indichiamo con $q_1,\dots,q_n$ e che determinano univocamente il
sistema tenendo conto dei vincoli. Le $q_i$ sono dette \emph{coordinate
  generalizzate} o \emph{lagrangiane}, non hanno necessariamente le dimensioni
di una lunghezza e la loro scelta non è univoca; $n$, il numero di grandezze
indipendenti, è chiamato \emph{numero di gradi di libertà}. D'ora in poi
indicheremo per brevità con $\bm{q} = (q_1,\dots,q_n)$ l'insieme delle $n$
coordinate generalizzate e con $\dot{\bm{q}} =
(\dot{q}_1,\dots,\dot{q}_n)$ l'insieme delle \emph{velocità
  generalizzate} sebbene, come già osservato, a rigore non tutte queste
grandezze hanno necessariamente la stessa dimensione fisica. Definiamo la
...
corpi che costituiscono il sistema sono date rispettivamente da
\begin{subequations}
  \begin{align}
...
massa totale e la massa ridotta già introdotti nel
...
di massa $M_\textup{T}$ e la cui posizione è quella del centro di massa, il
secondo termine è l'energia cinetica di un corpo fittizio di massa $\mu$ e
...
numero di coordinate indipendenti che lo individuano è al numero di gradi di
libertà del sistema. L'equazione di Lagrange per la coordinata $\bm{r}$ è
\begin{equation}
...
esse, quindi l'hamiltoniana del sistema rappresenta esattamente l'energia
meccanica totale
\begin{equation}
...
che la forza che agisce sulla particella di massa ridotta è centrale quindi,
come abbiamo visto nel paragrafo~\ref{sec:problema-keplero}, il vettore momento
...
il vettore $\bm{r}$ che individua la posizione della particella di massa ridotta
è legato ai vettori posizione $\bm{r}_1$ e $\bm{r}_2$ delle particelle di massa
...
\begin{figure}
  ...
  \caption{Orbite ellittiche dei due corpi e della particella fittizia di massa
    $\mu$ nel sistema di riferimento del centro di massa. Abbiamo posto $m_1=1$
    e $m_2=5$ in unità arbitrarie. I parametri dell'ellisse sono $a=1$ ed
    $e=0.8$.}
