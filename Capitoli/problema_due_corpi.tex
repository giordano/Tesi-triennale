...
\begin{figure}%TODO: completare la figura o rifarla
  \centering
  \input{Immagini/gnuplot/ellisse_vlos}
  \caption{Velocità nella direzione della linea di vista}
...
ma molto spesso è possibile misurare la sua componente nella direzione di vista
dell'osservatore $v_\textup{los}$. Supponiamo che l'osservatore guardi il
...
seguenti osservazioni geometriche. Chiamiamo $\alpha(\theta)$ l'angolo compreso
fra la direzione di $\versor{r}(\theta)$ e del vettore $\dot{\bm{r}}(\theta)
\equiv \bm{v}(\theta)$ e sia per il momento $\theta \in \mathopen{[}0,
\pi\mathclose{]}$. La tangente di questo angolo è data dal rapporto fra la
componente tangenziale $r(\theta)\dot{\theta}$ della velocità e la componente
radiale $\dot{r}(\theta)$, quindi
\begin{equation}
...
valore giusto di $\alpha(\theta)$ in questo intervallo dobbiamo allora
aggiungere $\pi$ al risultato dato dalla~\eqref{eq:angolo-alpha}, quindi in
definitiva
\begin{equation}
...
$\versor{x}$. Poiché $\alpha(\theta)$ è l'angolo fra il verso positivo della
direzione radiale e il vettore $\bm{v}(\theta)$ e la direzione radiale forma un
angolo $\theta$ con la direzione positiva dell'asse $\versor{x}$ abbiamo
$\beta(\theta) = \theta + \alpha(\theta)$ e la componente della velocità nella
direzione della linea di vista è
\begin{figure}
  \centering
...
circolari, sia ha sempre $\alpha(\theta) = \pi/2$, coerentemente col fatto che
