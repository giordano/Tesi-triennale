...
veda~\cites{bradt:processes}{fabrizio:meccanica}{smart:textbook}) e quelli
lagrangiano e hamiltoniano (si
veda~\cites{goldstein:meccanica}{landau:meccanica}), di questi ultimi daremo una
...
\begin{subequations}
  \begin{align}
    ...
    \frac{l_0}{\mu p}(1+e\cos(\theta-\theta_0)).
  \end{align}
\end{subequations}
...
figura~\ref{fig:ellisse-vlos}, quindi $v_\textup{los} = \bm{v} \cdot
\versor{x}$.
...
\pi/2\mathclose{[}$, ma nell'intervallo $\chi \in \mathopen{]}\pi,
2\pi\mathclose{[}$ la componente radiale $\dot{r}(\chi)$ assume valori
negativi, come si può vedere dalla~\eqref{eq:velocita-radiale} e nella
figura~\ref{fig:ellisse-vlos}, quindi l'angolo $\alpha(\chi)$ che la velocità
forma con la direzione radiale positiva è maggiore di $\pi/2$. Per ottenere il
valore giusto di $\alpha(\chi)$ in questo intervallo dobbiamo aggiungere $\pi$
...
\begin{figure}
  ...
  vera]{Andamento della velocità dei due corpi lungo la direzione della linea di
    ...
    $a = \SI{5e7}{\kilo\metre}$, $e=0.5$ e $m_2/m_1=5$}
...
$v_\textup{CM} = \SI{40}{\kilo\metre \per\second}$.
