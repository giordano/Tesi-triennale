...
\begin{figure}
  ...
  \begin{tikzpicture}[tdplot_main_coords,font=\footnotesize,scale=3]
    ...
    \draw [->] (O) -- node[left,fill=white] {$\bm{r}_1$} (\xuno,\yuno,\zuno)
               node[above] {$m_1$}; % vettore r_1
    \draw [->] (O) -- node[below right,fill=white] {$\bm{r}_2$}
               (\xdue,\ydue,\zdue) node[right] {$m_2$}; % vettore r_2
    % vettore posizione relativa
    \draw [->] (\xuno,\yuno,\zuno) -- node[above] {$\bm{r}$}
    (\xdue,\ydue,\zdue);
    % vettore posizione del centro di massa
    \draw [->] (O) -- node[right] {$\bm{r}_\textup{CM}$}
               (\xcdm,\ycdm,\zcdm);
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}[tdplot_main_coords,scale=3,font=\scriptsize]
    ...
    % lunghezza del vettore momento angolare. metto un valore arbitrario
    \pgfmathsetmacro{\l}{0.6}
    \pgfmathsetmacro{\m}{0.1} % massa
    ...
    \pgfmathdeclarefunction{mv}{1}{\pgfmathparse{\m*(2/dist(#1)-1/\a)}}
    ...
    %% primo punto
    \draw[->,thick] (O) -- node[above] {$\bm{r}_1$} (\xuno,\yuno); % posizione
    % vettore quantità di moto
    \draw[->,thick] (\xuno,\yuno) -- node[below] {$\bm{p}_1$} +(\pxuno,\pyuno);
    % vettore momento angolare
   \draw[->,thick] (\xuno,\yuno,0) -- node[left] {$\bm{l}_0$} +(0,0,\l);

    % %% secondo punto
    \draw[->,thick] (O) -- node[above] {$\bm{r}_2$} (\xdue,\ydue); % posizione
    % vettore quantità di moto
    \draw[->,thick] (\xdue,\ydue) -- node[below] {$\bm{p}_2$} +(\pxdue,\pydue);
    % vettore momento angolare
    \draw[->,thick] (\xdue,\ydue,0) -- node[right] {$\bm{l}_0$} +(0,0,\l);
  \end{tikzpicture}
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[scale=2.5,font=\footnotesize,
    ...
    % loro congrui in modulo 360
    \pgfmathdeclarefunction{vel}{1}{\pgfmathparse{(2/dist(#1)-1/\a)}}
    ...
    %% parametri del primo punto
    % anomalia vera del primo punto
    \pgfmathsetmacro{\anomaliauno}{100}
    % coordinata x del primo punto in cui disegno il vettore posizione
    \pgfmathsetmacro{\xuno}{dist(\anomaliauno)*cos(\anomaliauno)}
    % coordinata y del primo punto in cui disegno il vettore posizione
    \pgfmathsetmacro{\yuno}{dist(\anomaliauno)*sin(\anomaliauno)}
    ...
    %% secondo punto
    % anomalia vera del secondo punto
    \pgfmathsetmacro{\anomaliadue}{240}
    % coordinata x del secondo punto in cui disegno il vettore posizione
    \pgfmathsetmacro{\xdue}{dist(\anomaliadue)*cos(\anomaliadue)}
    % coordinata y del secondo punto in cui disegno il vettore posizione
    \pgfmathsetmacro{\ydue}{dist(\anomaliadue)*sin(\anomaliadue)}
    ...
    % origine dei sistemi di riferimento
    \node [shape=coordinate](O) at (0,0) [label=below left:$O \equiv
    M_{\textup{T}}$] {};
    % asse x
    ...
    %% secondo punto
    % vettore velocità
    \draw[->,vel] (\xdue,\ydue) -- +(\vxdue,\vydue) node[right] {$\bm{v}_2$};
    % vettore velocità radiale
    \draw[->,rad] (\xdue,\ydue) -- +({velrad(\anomaliadue)*cos(\anomaliadue)},
    {velrad(\anomaliadue)*sin(\anomaliadue)})
    node[above right] {$\bm{v}_{\textup{r},2}$};
    % vettore velocità tangenziale
    \draw[->,tang] (\xdue,\ydue) --
    +({veltang(\anomaliadue)*cos(\anomaliadue+90)},
    {veltang(\anomaliadue)*sin(\anomaliadue+90)}) node[below]
    {$\bm{v}_{\textup{t},2}$};
    % vettore velocità nella direzione della linea di vista (asse x)
    \draw[->,los] (\xdue,\ydue) --  +(\vxdue,0) node[right]
    {$\bm{v}_{\textup{los},2}$};
